...
export class AnalyticsService {
  ...
  static async trackEvent(eventData) {
    ...
    const requiredFields = ["eventType", "advertisementId"];
    for (const field of requiredFields) {
      if (!eventData[field]) {
        throw new Error(`${field} is required for tracking events`);
      }
    }

    // Ensure proper payload structure
    const payload = {
      eventType: eventData.eventType, // "IMPRESSION", "CLICK", "VIEW"
      advertisementId: eventData.advertisementId,
      sessionId: eventData.sessionId || null,
      ipAddress: eventData.ipAddress || null,
      userAgent: eventData.userAgent || null,
      deviceType: eventData.deviceType || null,
      browserType: eventData.browserType || null,
      locationCountry: eventData.locationCountry || null,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to track event: ${response.status} - ${errorData}`
        );
      }
  ...
  static async getDashboardMetrics(credentials) {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.text();
        throw new Error(
          `Failed to fetch dashboard metrics: ${response.status} - ${errorData}`
        );
      }
  ...
  static async getAdvertisementMetrics(advertisementId, credentials) {
    try {
      if (!advertisementId) {
      ...
      // Use proxy route to avoid CORS issues in development
      const response = await fetch(
        `${ANALYTICS_PROXY_BASE}/metrics/${advertisementId}`,
        {
          method: "GET",
          headers,
        }
      ...
      if (!response.ok) {
        const errorData = await response.text();
        throw new Error(
          `Failed to fetch advertisement metrics: ${response.status} - ${errorData}`
        );
      }
  ...
  static validateEventData(eventData) {
    ...
    if (
      !Number.isInteger(eventData.advertisementId) ||
      eventData.advertisementId <= 0
    ) {
  ...
  /**
   * Create a complete event payload with automatic detection of browser/device info
   * @param {string} eventType - Type of event ("IMPRESSION", "CLICK", "VIEW")
   * @param {number} advertisementId - ID of the advertisement
   * @param {Object} [overrides={}] - Optional overrides for any field
   * @returns {Object} Complete event payload
   */
  static createEventPayload(eventType, advertisementId, overrides = {}) {
    const defaultPayload = {
      eventType,
      advertisementId,
      sessionId: this.generateSessionId(),
      ipAddress: null, // Should be set by server or client if available
      userAgent: typeof navigator !== "undefined" ? navigator.userAgent : null,
      deviceType:
        typeof navigator !== "undefined" ? this.getDeviceType() : null,
      browserType:
        typeof navigator !== "undefined" ? this.getBrowserType() : null,
      locationCountry: null, // Should be set by geolocation service
      locationCity: null, // Should be set by geolocation service
  ...
  static async getDashboardSummary(credentials) {
    try {
      const metricsResult = await this.getDashboardMetrics(credentials);
